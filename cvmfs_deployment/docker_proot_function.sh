dockerrun()
{
  case "${SCRAM_ARCH}" in
    ...
    slc7_aarch64_* )
      ...
      $PROOTDIR/proot -R $PROOTDIR/centos-7.2.1511-aarch64-rootfs -b /tmp:tmp -b /build:/build -b /cvmfs:/cvmfs -w ${INSTALL_PATH} -q "$PROOTDIR/qemu-aarch64 -cpu cortex-a57" sh -c "${ARGS}"
      ;;
    fc24_ppc64le_* )
      ARGS="export INSTALL_PATH=${INSTALL_PATH}; export INSTALL_PATH=${INSTALL_PATH}; export SCRAM_ARCH=${SCRAM_ARCH}; export x=${x}; cd ${INSTALL_PATH}; $@"
      $PROOTDIR/proot -R $PROOTDIR/fedora-24-ppc64le-rootfs -b /tmp:/tmp -b /build:/build -b /cvmfs:/cvmfs -w ${INSTALL_PATH} -q "$PROOTDIR/qemu-ppc64le -cpu POWER8" sh -c "${ARGS}"
      ;;
    slc7_ppc64le_* )
      ARGS="export INSTALL_PATH=${INSTALL_PATH}; export INSTALL_PATH=${INSTALL_PATH}; export SCRAM_ARCH=${SCRAM_ARCH}; export x=${x}; cd ${INSTALL_PATH}; $@"
      $PROOTDIR/proot -R $PROOTDIR/centos-7.2.1511-ppc64le-rootfs -b /tmp:/tmp -b /build:/build -b /cvmfs:/cvmfs -w ${INSTALL_PATH} -q "$PROOTDIR/qemu-ppc64le -cpu POWER8" sh -c "${ARGS}"
      ;;
    * )
      eval $@
      ;;
